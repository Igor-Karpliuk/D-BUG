...
void PlayScene::Update()
{
	...
	if (toggleFollowPath == true)
	{
		if (!m_pPathList.empty())
			m_followPath();
	}
	else
		toggleFollowPath = false;
}
...
void PlayScene::m_moveGameObject(T*& object, const int col, const int row, const TileStatus status)
{
	...
	// only change status of Tile object is moving from if it is not an impassable
	...
	// only change status of the Tile object is moving to if it is not an impassable tile
	if (m_getTile(object->GetGridPosition())->GetTileStatus() != TileStatus::IMPASSABLE)
	{
...
void PlayScene::Start()
{
	...
	m_pStartLabel = new Label("Press F to find the shortest path", "Consolas", 20, blue, glm::vec2(200.0f, 20.0f));
	m_pStartLabel->SetParent(this);
	AddChild(m_pStartLabel);

	
	m_pStartLabel = new Label("Press R to reset the path", "Consolas", 20, blue, glm::vec2(600.0f, 20.0f));
	m_pStartLabel->SetParent(this);
	AddChild(m_pStartLabel);
	// Set GUI Title
...
void PlayScene::GUI_Function()
{
	...
	if (ImGui::CollapsingHeader("Tile Map"))
	{
		...
		for (int i = 0; i < m_tileMap.length(); ++i)
		{
			...
			{
				ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(0.850f, 0.325f, 0.309f, 1.0f)); // Bootstrap Warning - Red
			}
			else
			{
...
void PlayScene::m_resetPathFinding()
{
	...
	// reset oath found boolean
	m_pathFound = false;

	// mark all the tiles in the grid as UNIVSITED
	for (const auto tile : m_pGrid)
	{
...
void PlayScene::m_followPath()
{
	toggleFollowPath = true;
	if (!m_pPathList.empty())
	{
		static float timer = 0.0f;
		Tile* currentNode = m_pPathList.front();
		constexpr auto offset = glm::vec2(Config::TILE_SIZE * 0.5f, Config::TILE_SIZE * 0.5f);

		glm::vec2 nodePos = m_getTile(currentNode->GetGridPosition().x,
			currentNode->GetGridPosition().y)->GetTransform()->position + offset;

		glm::vec2 new_position = Util::Lerp(m_pStarShip->GetTransform()->position, nodePos, timer);

		glm::vec2 rotation = Util::Normalize(nodePos - m_pStarShip->GetTransform()->position);
		...
		m_pStarShip->GetTransform()->position = new_position;
	}
	else
		toggleFollowPath = false;
}