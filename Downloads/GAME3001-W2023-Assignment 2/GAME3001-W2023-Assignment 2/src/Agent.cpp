...
Agent::Agent()
{
	...
	m_lineColour[2] = glm::vec4(0, 0, 1, 1); // right line colour 
	m_lineColour[3] = glm::vec4(0, 1, 0, 1); // far left line colour
	m_lineColour[4] = glm::vec4(0, 1, 0, 1); // far right line colour

	// initialize whisker collision booleans
...
void Agent::SetLeftLeftLOSEndPoint(glm::vec2 point)
{
	m_leftleftLOSEndPoint = point;
}

void Agent::SetRightRightLOSEndPoint(glm::vec2 point)
{
...
void Agent::UpdateWhiskers(const float angle)
{
	...
	// Left Left Whisker
	x = sin((GetCurrentHeading() - (m_whiskerAngle + 45.0f) + 90.0f) * Util::Deg2Rad);
	y = cos((GetCurrentHeading() - (m_whiskerAngle + 45.0f) + 90.0f) * Util::Deg2Rad);
